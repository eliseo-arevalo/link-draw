...
describe("drawing-links", () => {
  ...
  describe("creation functions", () => {
    ...
    it("should create element link", () => {
      expect(createElementLink(TEST_DRAWING_ID, TEST_ELEMENT_ID)).toBe(
        `${DRAWING_LINK_PREFIX}${TEST_DRAWING_ID}#element:${TEST_ELEMENT_ID}`
      )
    })

    it("should create frame link", () => {
      expect(createFrameLink(TEST_DRAWING_ID, TEST_FRAME_ID)).toBe(
        `${DRAWING_LINK_PREFIX}${TEST_DRAWING_ID}#frame:${TEST_FRAME_ID}`
      )
    })
  })
  ...
  describe("parseDrawingLink", () => {
    ...
    it("should fallback to drawing type if fragment is invalid", () => {
      expect(parseDrawingLink(`${DRAWING_LINK_PREFIX}${TEST_DRAWING_ID}#invalid`)).toEqual({
        type: "drawing",
        drawingId: TEST_DRAWING_ID,
      })
    })
    ...
    it("should fallback to drawing type if fragment value is missing", () => {
      expect(parseDrawingLink(`${DRAWING_LINK_PREFIX}${TEST_DRAWING_ID}#element:`)).toEqual({
        type: "drawing",
        drawingId: TEST_DRAWING_ID,
      })
    })
  ...
  describe("drawingLinkToUrl", () => {
    it("should convert to editor URL", () => {
      expect(drawingLinkToUrl(createDrawingLink(TEST_DRAWING_ID))).toBe(
        `/editor?id=${TEST_DRAWING_ID}`
      )
    })
  ...
  describe("getLinkDescription", () => {
    ...
    it("should describe element link with truncated ID", () => {
      ...
      expect(getLinkDescription(createElementLink(TEST_DRAWING_ID, longId))).toContain(
        "12345678..."
      )
    })

    it("should describe frame link", () => {
      expect(getLinkDescription(createFrameLink(TEST_DRAWING_ID, "f1"))).toContain(
        "Navigate to frame"
      )
    })
